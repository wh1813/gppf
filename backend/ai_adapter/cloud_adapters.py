...
class UniversalOpenAIAdapter(BaseAIAdapter):
    ...
    async def analyze(self, request: StockAnalysisRequest) -> StockAnalysisResult:
        ...
        try:
            response = await self.client.chat.completions.create(
                model=self.model_name, messages=messages, temperature=0.1, max_tokens=1500
            )
            raw_content = response.choices[0].message.content
            return self._parse_response(raw_content, request.symbol, self.model_name)
        except Exception as e:
            print(f"[{self.model_name}] API 调用失败: {e}")
            raise e
...
class GeminiAdapter(BaseAIAdapter):
    ...
    async def analyze(self, request: StockAnalysisRequest) -> StockAnalysisResult:
        ...
        config = types.GenerateContentConfig(
            temperature=0.1, response_mime_type="application/json", system_instruction=self.system_instruction
        )
        try:
            response = await self.client.aio.models.generate_content(
                model=self.model_name, contents=contents, config=config
            )
            return self._parse_response(response.text, request.symbol, self.model_name)
        except Exception as e:
